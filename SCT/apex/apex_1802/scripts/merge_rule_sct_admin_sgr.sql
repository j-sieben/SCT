set define ^

declare
  l_foo number;
begin
  l_foo := sct_admin.map_id;
  
  dbms_output.put_line('&s1.Rulegroup SCT_ADMIN_SGR');
  sct_admin.merge_rule_group(
    p_sgr_id => sct_admin.map_id(90),
    p_sgr_name => 'SCT_ADMIN_SGR',
    p_sgr_description => q'|Hauptseite der SCT-Administration|',
    p_sgr_app_id => coalesce(apex_application_install.get_application_id, ^APP_ID.),
    p_sgr_page_id => 1,
    p_sgr_with_recursion => 1,
    p_sgr_active => 1);
  
  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(92),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Regelgruppe bekannt',
    p_sru_condition => q'|P1_SGR_ID is not null|',
    p_sru_sort_seq => 30,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(106),
    p_sra_sru_id => sct_admin.map_id(92),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'B1_COPY_RULE_GROUP',
    p_sra_sat_id => 'SHOW_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(108),
    p_sra_sru_id => sct_admin.map_id(92),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'R1_RULE_OVERVIEW',
    p_sra_sat_id => 'REFRESH_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 20,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(94),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Anwendungsfilter ist nicht leer',
    p_sru_condition => q'|P1_SGR_APP_ID is not null|',
    p_sru_sort_seq => 50,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(110),
    p_sra_sru_id => sct_admin.map_id(94),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_ID',
    p_sra_sat_id => 'EMPTY_FIELD',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(112),
    p_sra_sru_id => sct_admin.map_id(94),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_PAGE_ID',
    p_sra_sat_id => 'REFRESH_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 20,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(96),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Anwendungsfilter ist leer',
    p_sru_condition => q'|P1_SGR_APP_ID is null|',
    p_sru_sort_seq => 40,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(114),
    p_sra_sru_id => sct_admin.map_id(96),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_ID',
    p_sra_sat_id => 'EMPTY_FIELD',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 30,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(116),
    p_sra_sru_id => sct_admin.map_id(96),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_PAGE_ID',
    p_sra_sat_id => 'DISABLE_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 20,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(118),
    p_sra_sru_id => sct_admin.map_id(96),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_PAGE_ID',
    p_sra_sat_id => 'EMPTY_FIELD',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(120),
    p_sra_sru_id => sct_admin.map_id(96),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'R1_RULE_GROUP',
    p_sra_sat_id => 'REFRESH_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 40,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(98),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Initialisierung',
    p_sru_condition => q'|initializing = 1|',
    p_sru_sort_seq => 10,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(122),
    p_sra_sru_id => sct_admin.map_id(98),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_ID',
    p_sra_sat_id => 'EMPTY_FIELD',
    p_sra_attribute => q'|''|',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(100),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Regelgruppe unbekannt',
    p_sru_condition => q'|P1_SGR_ID is null|',
    p_sru_sort_seq => 20,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(124),
    p_sra_sru_id => sct_admin.map_id(100),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'B1_COPY_RULE_GROUP',
    p_sra_sat_id => 'DISABLE_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(126),
    p_sra_sru_id => sct_admin.map_id(100),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'R1_RULE_OVERVIEW',
    p_sra_sat_id => 'REFRESH_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 20,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(102),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Seitenfilter ist nicht leer',
    p_sru_condition => q'|P1_SGR_PAGE_ID is not null|',
    p_sru_sort_seq => 70,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(128),
    p_sra_sru_id => sct_admin.map_id(102),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'P1_SGR_ID',
    p_sra_sat_id => 'EMPTY_FIELD',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);

  sct_admin.merge_rule(
    p_sru_id => sct_admin.map_id(104),
    p_sru_sgr_id => sct_admin.map_id(90),
    p_sru_name => 'Seitenfilter ist leer',
    p_sru_condition => q'|P1_SGR_PAGE_ID is null|',
    p_sru_sort_seq => 60,
    p_sru_fire_on_page_load => -1,
    p_sru_active => 1);

  
  sct_admin.merge_rule_action(
    p_sra_id => sct_admin.map_id(130),
    p_sra_sru_id => sct_admin.map_id(104),
    p_sra_sgr_id => sct_admin.map_id(90),
    p_sra_spi_id => 'R1_RULE_GROUP',
    p_sra_sat_id => 'REFRESH_ITEM',
    p_sra_attribute => q'||',
    p_sra_attribute_2 => q'||',
    p_sra_sort_seq => 10,
    p_sra_on_error => 0,
    p_sra_raise_recursive => 1,
    p_sra_active => 1);
  
  commit;
end;
/
set define &