  ...
  with params as (
       ...
       select sru.rowid row_id, sru.sru_id, p.sgr_id,
              ...
              case
                ...
                when sra.sra_on_error = 1 then p.span_on_error || p.sgr_page_prefix || sra_spi_id || ': ' || sat_name || p.close_span
         ...
         left join sct_page_item spi
          ...
          and spi.spi_has_error = 1
         left join (
              select sra.sra_sgr_id, 
                     case when sra.sra_spi_id = 'DOCUMENT' and to_char(sra.sra_attribute_2) is not null then '[' || to_char(sra.sra_attribute_2) || ']' else sra.sra_spi_id end sra_spi_id,
