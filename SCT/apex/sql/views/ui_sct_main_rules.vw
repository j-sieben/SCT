...
with params as (
       ...
       select sru.rowid row_id, sru.sru_id, p.sgr_id, sru.sru_name, p.sgr_app_id, p.sgr_page_id,
              replace(replace(dbms_lob.substr(sru.sru_condition, 4000, 1), ' and ', '<br>and '), ' or ', '<br>or ') sru_condition,
              ...
              case sra_active
              when 1 then p.sgr_page_prefix || sra_spi_id || ': ' || sat_name
              else '<span class="ek-tool-disabled">' || p.sgr_page_prefix || sra_spi_id || ': ' || sat_name || '</span>' end sra_name,
              case sru.sru_active when 1 then '<i class="fa fa-check"/>' else '<i class="fa fa-times">' end sru_active
         from params p
         join sct_rule sru
           on p.sgr_id = sru.sru_sgr_id
         left join sct_rule_action sra
...
select a.row_id,
 ...
 group by a.row_id, app.application_id, app.application_name || ' (' || app.application_id || ')',
       ...
       a.sru_id, a.sgr_id, a.sru_name, a.sru_condition, a.sru_firing_items, a.sru_sort_seq, sru_active;