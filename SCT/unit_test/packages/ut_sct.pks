create or replace package ut_sct
  authid definer
as
  ...
  --%test (identifies 3 well known items as relevant when rendering page ADMIN_SGR of the SCT application)
  procedure render_plugin;
  
  --%test (executes 1 rule with 1 action if setting item P1_SGR_ID to the ID of SCT_ADMIN_SGR rule group)
  procedure refresh_plugin;
  
  --%endcontext
  
  --%context(SCT core functionality)
