...
(function (sct, $, server) {
   ...
   * of the button must not be processed anymore. But because these events appear so quickly, SCT is unable to respond before the 
  ...
  function bindEvent(item, event, action) {
    ...
    if ($this.length > 0) {
	  ...
	  // SCT unbinds event handlers bound to this item to prevent problems between the different handlers
	  $this
	    .off(event)
   ...
   * @public
   */
  sct.setItemLabel = function (itemID, label){
    sct.ApexJS.setItemLabel(itemID, label);
  };
   ...
   * @public
   */
  sct.hideFilterPanel = function (item) {
    forEach(item, function () {
      var $this = $(this);
      var itemId = $this.attr('id');
      sct.ApexJS.hideFilterPanel(itemId);
    });
  };
   ...
   * @public
   */
  sct.persistIGSelection = function(ig, item, column){
	  var id = item;
