...
as
  ...
  -- um anschlieﬂend auch fuer diese Elemente die Regelpruefung aufzurufen.
  ...
  begin
    ...
    -- Wird benoetigt, um ªbreadth first´ zu arbeiten: Alle Rekursionsaufrufe einer Ebene, danach die naechste Ebene etc.
    ...
    while g_param.firing_item is not null loop
      begin
        --  fuehre alle ªEvents´ auf aktuellem Rekursionslevel aus
        if g_param.recursive_stack(g_param.firing_item) = l_actual_recursive_level then
          ...
          l_js_action_chunk := utl_text.bulk_replace(l_js_action_chunk, char_table(
                                ...
                                '#TIME#', l_elapsed, -- hier wird die Zeit eingef¸gt
