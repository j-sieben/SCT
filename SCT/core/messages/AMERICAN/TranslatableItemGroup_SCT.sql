...
^'
  ...
  pit_admin.merge_translatable_item(
    ...
    p_pti_display_name => q'^^',
    p_pti_description => q'^<p>Ensures that a confirmation message is displayed when a button is clicked.<br>Only when this request is confirmed is the event reported to SCT.</p>^'
  );
  ...
  pit_admin.merge_translatable_item(
    ...
    p_pti_display_name => q'^^',
    p_pti_description => q'^<p>If the event is triggered, this is reported to SCT and can be caught with a rule DIALOG_CLOSED = 1. The element that registers as FIRING_ITEM in this action is the element that is sent.</p>^'
  );
  ...
  pit_admin.merge_translatable_item(
    ...
    p_pti_display_name => q'^^',
    p_pti_description => q'^<p>If the event is triggered, this is reported to SCT and can be caught with a rule DOUBLE_CLICK = 1. <br>The triggering element is the element that is registered as FIRING_ITEM in this action.</p>^'
  );
  ...
  pit_admin.merge_translatable_item(
    ...
    p_pti_display_name => q'^^',
    p_pti_description => q'^<p>Registers an Enter key event handler.<br>If the event is triggered, this is reported to SCT and can be caught with a ruleENTER_KEY = 1.<br>Trigging element is the element that is registered as FIRING_ITEM in this action.</p>^'
  );
...
^'
  ...
  pit_admin.merge_translatable_item(
    ...
    p_pti_display_name => q'^^',
    p_pti_description => q'^<p>Action observes a field or class and registers the elements so that the corresponding element values are copied to the session state at each event on the page.<br>Required only if no SCT rule contains this item or class.</p>^'
  );
