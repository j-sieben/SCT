...
begin
  utl_text.merge_template(
    ...
    p_uttm_type => 'SCT',
    p_uttm_mode => 'ACTION',
    p_uttm_text => q'{{"name":"#SAA_NAME#"#SAA_LABEL|,^CR^"label":"|"|##SAA_LABEL_KEY|,^CR^"labelKey":"|"|##SAA_CONTEXT_LABEL|,^CR^"contextLabel":"|"|##SAA_ICON|,^CR^"icon":"|"|##SAA_ICON_TYPE|,^CR^"iconType":"|"|##SAA_INITIALLY_DISABLED|,^CR^"disabled":||##SAA_INITIALLY_HIDDEN|,^CR^"hide":||##SAA_TITLE|,^CR^"title":"|"|##SAA_SHORTCUT|,^CR^"shortcut":"|"|##SAA_HREF|,^CR^"href":"|"|##SAA_ACTION|,^CR^"action":"|"|#}}',
    p_uttm_log_text => q'{}',
...
q'{ order by sru.sru_sort_seq desc, srg.sra_sort_seq}',
  ...
  utl_text.merge_template(
    ...
    p_uttm_text => q'{  sct_admin.merge_apex_action_type(\CR\}' || 
q'{    p_sty_id => '#STY_ID#',\CR\}' || 
q'{    p_sty_name => '#STY_NAME#',\CR\}' || 
q'{    p_sty_description => #STY_DESCRIPTION#,\CR\}' || 
q'{    p_sty_active  => #STY_ACTIVE#);\CR\}' || 
q'{}',
    p_uttm_log_text => q'{}',
    p_uttm_log_severity => 70
  );
  ...
  utl_text.merge_template(
    ...
    p_uttm_text => q'{  sct_admin.merge_page_item_type(\CR\}' || 
q'{    p_sit_id => '#SIT_ID#',\CR\}' || 
q'{    p_sit_name => '#SIT_NAME#',\CR\}' || 
q'{    p_sit_has_value => #SIT_HAS_VALUE#,\CR\}' || 
q'{    p_sit_include_in_view => #SIT_INCLUDE_IN_VIEW#,\CR\}' || 
q'{    p_sit_event => '#SIT_EVENT#',\CR\}' || 
q'{    p_sit_col_template => #SIT_COL_TEMPLATE#,\CR\}' || 
q'{    p_sit_init_template => #SIT_INIT_TEMPLATE#);\CR\}' || 
q'{}',
    p_uttm_log_text => q'{}',
    p_uttm_log_severity => 70
  );
...
q'{    p_sif_actual_page_only => #SIF_ACTUAL_PAGE_ONLY#,\CR\}' || 
q'{    p_sif_item_types => '#SIF_ITEM_TYPES#',\CR\}' || 
