...
select *
  from (select application_id, page_id, 'ITEM' target_type, item_name target_name,
               case
         ...
         union all
        select application_id, 0, 'APP_ITEM', item_name, q'~v('#ITEM#')~'
 ...
 union all 
select sgr_app_id, sgr_page_id, 'DOCUMENT', 'DOCUMENT', null
  from sct_rule_group;
